


#define SS_PIN 16

#define AUTOCONNECT_AP_SSID "ESP2866_AP"
#define AUTOCONNECT_AP_PASSWORD "D3@d1ock"

#define CARD_READER_TIMEOUT 5000
#define MAIN_LOOP_DELAY 500

#define TOKEN_ENDPOINT "https://web-app2.uniba.sk/students-backend/token/"
#define TOKEN_REQUEST_DATA "username=vita3&password=secret"
#define TOKEN_TIMEOUT 60000

#define LOG_ENDPOINT "https://web-app2.uniba.sk/students-backend/log/"
#define TEST_LOG_TIMEOUT 20000

#define ACCESS_ENDPOINT_BASE "https://web-app2.uniba.sk/students-backend/entry/eval/"
